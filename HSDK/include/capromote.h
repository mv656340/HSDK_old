//------------------------------------------------------+-----------------------------------------------
// FILENAME:           capromote.h
//
// DESCRIPTION:        Declares the CaPromote class.
//
...
//
//  DESCRIPTION:
//      CaPromote is the HSDK class that represents the Harvest Promote object.
//
//  NOTES:
//      1) None.
//
...
class HSDKDLL_CLASS_DECLSPEC CaPromote 
{
    private:

        // Member Variables
		CPtHPromote*	m_pHPromote;					// Hapi Promote Object
		CaHarvest*		m_pCaHarvest;				// HSDK Harvest pointer
     
		// Private member functions
        void            Init( void );
        CaPromote(CaHarvest &rHarvest);								// Can only be created by CaContext

    public:
        // Constructors

        // Destructor
        virtual ~CaPromote(void);

        // Public Member Functions
        bool                SetMergedPkgsOnly(bool bFlag);
        bool                SetCheckPackageDependencies(bool bCheckPackageDependencies);
        bool                SetCheckPackageGroupBind( bool bFlag);
        bool                GetMergedPkgsOnly(void) const;
...
//PR#8216, e.Deactivate
	...
	// pr3214 har521sp1 suse8, gcc3-- lisla01 ostream in std namespace
        friend HSDKDLL_CLASS_DECLSPEC std::ostream & operator<<( std::ostream &o, const CaPromote &Context );
#else
        friend HSDKDLL_CLASS_DECLSPEC ostream & operator<<( ostream &o, const CaPromote &Context );
#endif
